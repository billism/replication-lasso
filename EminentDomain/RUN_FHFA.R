...
#### Import Data
#### ================================================================
URL1  <- "./data/temp_HomePrices_FHFA.mat.csv"
URL2  <- "./data/probs.csv"

FHFA <- read.csv(URL1)                   # CSHomePriceIndex
...
## dim(FHFA)
## head(names(CHSPI))
## sum(sapply(FHFA, is.integer))
## sum(sapply(FHFA, is.double))
...
#### ================================================================
#runtime <- proc.time()
probs.new <- data.frame(matrix(nrow=nrow(FHFA), ncol=ncol(probs)-2))
colnames(probs.new) <- colnames(probs)[-(1:2)]

for (i in 1:nrow(FHFA)) {
  year          <- FHFA$year[i]
  circuit       <- FHFA$circuit[i]
...
#runtime <- proc.time()
#prr.new <- do.call(rbind, lapply(1:nrow(FHFA), function(i) return(probs[which((probs$Syear == FHFA$year[i]) & (probs$Scircuit == FHFA$circuit[i])), -(1:2)])))
#cat(proc.time() - runtime)
...
yearn <- as.factor(FHFA$year)
iyearn <- dummy(yearn)[,-1]
circuitn <- as.factor(FHFA$circuit)
...
jcircuityear <- icircuitn * FHFA$year
...
######## - circuit: index %/% 18
######## - year   : index %%  18
cy <- ((recode(FHFA$circuit) - 1) * max(recode(FHFA$year)) + recode(FHFA$year))
...
x <- data.matrix(data.frame("missing_cy_12" = FHFA$missing_cy_12,
                            "numcasecat_12" = FHFA$numcasecat_12,
                            "icircuitn" = icircuitn,
                            "iyearn" = iyearn,
...
###### ================================================================
d <- data.matrix(FHFA$numpro_casecat_12)

###### Dependent Variable (y)
###### ================================================================
y  <- data.matrix(FHFA$loghpi)

###### Candidate Instruments (z)
###### ================================================================
source("./instruments_FHFA.R")
z <- data.matrix(z)
