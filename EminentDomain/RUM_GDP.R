...
#### Import Data
#### ================================================================
URL1  <- "./data/temp_gdp.mat.csv"
URL2  <- "./data/probs.csv"

GDP <- read.csv(URL1)                   # CSHomePriceIndex
...
## dim(GDP)
## head(names(CHSPI))
## sum(sapply(GDP, is.integer))
## sum(sapply(GDP, is.double))
...
#### ================================================================
#runtime <- proc.time()
probs.new <- data.frame(matrix(nrow=nrow(GDP), ncol=ncol(probs)-2))
colnames(probs.new) <- colnames(probs)[-(1:2)]

for (i in 1:nrow(GDP)) {
  year          <- GDP$year[i]
  circuit       <- GDP$circuit[i]
...
#runtime <- proc.time()
#prr.new <- do.call(rbind, lapply(1:nrow(GDP), function(i) return(probs[which((probs$Syear == GDP$year[i]) & (probs$Scircuit == GDP$circuit[i])), -(1:2)])))
#cat(proc.time() - runtime)
...
yearn <- as.factor(GDP$year)
iyearn <- dummy(yearn)[,-1]
circuitn <- as.factor(GDP$circuit)
...
jcircuityear <- icircuitn * GDP$year
...
######## - circuit: index %/% 18
######## - year   : index %%  18
cy <- ((recode(GDP$circuit) - 1) * max(recode(GDP$year)) + recode(GDP$year))
...
x <- data.matrix(data.frame("missing_cy_12" = GDP$missing_cy_12,
                            "numcasecat_12" = GDP$numcasecat_12,
                            "icircuitn" = icircuitn,
                            "iyearn" = iyearn,
...
###### ================================================================
d <- data.matrix(GDP$numpro_casecat_12)

###### Dependent Variable (y)
###### ================================================================
y  <- data.matrix(GDP$loggdp)

###### Candidate Instruments (z)
###### ================================================================
source("./instruments_gdp.R")
z <- data.matrix(z)
...
cat(paste0(sum(IND_dan), " instruments selected by Daniel!\n"))
if (sum(IND_dan) != 0){
  for(rname in rownames(IND_dan)[IND_dan]) {
    cat(paste0(">>> ", rname, "\n"))
  }
}
...
cat(paste0(sum(INDunion), " instruments will be used!\n"))
for(rname in rownames(INDunion)[INDunion]) {
  cat(paste0(">>> ", rname, "\n"))
}
