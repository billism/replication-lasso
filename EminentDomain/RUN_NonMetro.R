...
#### Import Data
#### ================================================================
URL1  <- "./data/temp_HomePrices_nonmetro.mat.csv"
URL2  <- "./data/probs.csv"

NonMetro <- read.csv(URL1)                   # CSHomePriceIndex
...
## dim(NonMetro)
## head(names(CHSPI))
## sum(sapply(NonMetro, is.integer))
## sum(sapply(NonMetro, is.double))
...
#### ================================================================
#runtime <- proc.time()
probs.new <- data.frame(matrix(nrow=nrow(NonMetro), ncol=ncol(probs)-2))
colnames(probs.new) <- colnames(probs)[-(1:2)]

for (i in 1:nrow(NonMetro)) {
  year          <- NonMetro$year[i]
  circuit       <- NonMetro$circuit[i]
...
#runtime <- proc.time()
#prr.new <- do.call(rbind, lapply(1:nrow(NonMetro), function(i) return(probs[which((probs$Syear == NonMetro$year[i]) & (probs$Scircuit == NonMetro$circuit[i])), -(1:2)])))
#cat(proc.time() - runtime)
...
yearn <- as.factor(NonMetro$year)
iyearn <- dummy(yearn)[,-1]
circuitn <- as.factor(NonMetro$circuit)
...
jcircuityear <- icircuitn * NonMetro$year
...
######## - circuit: index %/% 18
######## - year   : index %%  18
cy <- ((recode(NonMetro$circuit) - 1) * max(recode(NonMetro$year)) + recode(NonMetro$year))
...
x <- data.matrix(data.frame("missing_cy_12" = NonMetro$missing_cy_12,
                            "numcasecat_12" = NonMetro$numcasecat_12,
                            "icircuitn" = icircuitn,
                            "iyearn" = iyearn,
...
###### ================================================================
d <- data.matrix(NonMetro$numpro_casecat_12)

###### Dependent Variable (y)
###### ================================================================
y  <- data.matrix(NonMetro$loghpi_nonmetro)

###### Candidate Instruments (z)
###### ================================================================
source("./instruments_nonmetro.R")
z <- data.matrix(z)
