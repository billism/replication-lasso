# Simulation : Cut-Off Design with s=5
#########################################
## 2SLS
## Fuller
## Post-Lasso
...
source("./tsls.R")
source("./fuller.R")
sourceCpp("./lasso_shooting_cpp.cpp")
...
for (ii in 1:nn) {
  for (jj in 1:ncp) {
    ...
    for (kk in 1:nSims) {
      ...
      # 
      # if ( max(b_tmp) < 1e-4) {
      #   cat(paste0("Ridge Estimation Too Small!\n"))
      ...
      b_tmp <- lasso_shooting_cpp(Z,X, lambda)$
      ind0 <- abs(b_tmp) > 0
      Z0 <- as.matrix(Z[,ind0])
      Z1 <- Z0
      ind1 <- ind0
      ...
      #### Cross Validation for Ridge Penalty
...
write.xlsx(All_Results, file = "./Sim_Output_Cutoff_05.xlsx",
